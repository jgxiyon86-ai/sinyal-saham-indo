...
class ClientPageController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $summary = [
            ...
            'active' => User::query()->where('role', 'client')->where('is_active', true)->count(),
            'inactive' => User::query()->where('role', 'client')->where('is_active', false)->count(),
        ];
    ...
    public function toggleActive(Request $request, User $client): RedirectResponse
    {
        abort_unless($client->role === 'client', 404);
        ...
        $status = $next
            ? "Klient {$client->name} diaktifkan kembali."
            : "Klient {$client->name} dinonaktifkan. Tidak akan menerima WA blast/sinyal aplikasi.";
